...
---
Modified by: Justin Ruan
Discription:
    Copyright(c) 2024-2025 Basic Lab @ NYCU
    Follow the same license as the original code
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ROOT_DIR",
        type=str,
        default=osp.abspath(osp.dirname(osp.dirname(osp.realpath(__file__)))),
    )

    parser.add_argument("--max_episode_step", type=int, default=300)
    parser.add_argument("--auto_ego", action="store_true")
    parser.add_argument(
        "--mode", "-m", type=str, default="eval", choices=["train_scenario", "train_agent", "eval"]
    )
    parser.add_argument("--agent_cfg", nargs="*", type=str, default=["adv_scenic.yaml"])
    parser.add_argument(
        "--scenario_cfg", nargs="*", type=str, default=["train_scenario_scenic.yaml"]
    )
    ...
    parser.add_argument(
        "--num_scenario", "-ns", type=int, default=1, help="num of scenarios we run in one episode"
    )
    ...
    parser.add_argument(
        "--host", type=str, default="127.0.0.1", help="host to communicate with carla"
    )
    ...
    for agent_cfg in args.agent_cfg:
        for scenario_cfg in args.scenario_cfg:
            ...
            agent_config["load_dir"] = osp.join(
                agent_config["load_dir"], f"scenario_{scenario_config['scenario_id']}"
            )
            ...
            args_dict["output_dir"] = osp.join(
                "log",
                "adv_train",
                args.mode,
                agent_config["policy_name"],
                f"{agent_cfg.split('.')[0]}",
