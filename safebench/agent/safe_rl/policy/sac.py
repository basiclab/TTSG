...
class SAC(Policy):
    ...
    def _ac_training_setup(self, actor, critic):
        ...
        self.actor, self.critic, self.critic_targ = to_device(
            [actor, critic, critic_targ], get_device_name()
        )
    ...
    def _update_critic(self, data):
        def critic_loss():
            obs, act, reward, obs_next, done = (
                to_tensor(data["state"]),
                to_tensor(data["action"]),
                to_tensor(data["reward"]),
                to_tensor(data["n_state"]),
                to_tensor(data["done"]),
            )
            ...
            with torch.no_grad():
                ...
                act_next, logp_a_next = self.actor_forward(
                    obs_next, deterministic=False, with_logprob=True
                )
                # Target Q-values
