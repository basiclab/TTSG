...
class TD3_BEV(Policy):
    def __init__(
        ...
        **kwargs,
        ...
        self.timer = (
            0  # used to log how many updating steps and help to delay the policy update
        )
        ...
        if ac_model.lower() == "mlp":
            ...
            if isinstance(env.action_space, gym.spaces.Box):
                actor = MLPActor(
                    self.obs_dim, self.act_dim, hidden_sizes, nn.ReLU, self.act_lim
                )
            elif isinstance(env.action_space, gym.spaces.Discrete):
                raise ValueError("Discrete action space does not support yet")
            critic = EnsembleQCritic(
                self.obs_dim, self.act_dim, hidden_sizes, nn.ReLU, num_q=num_q
            )
        else:
    ...
    def _ac_training_setup(self, actor, critic, encoder):
        ...
        (
            ...
            [actor, actor_targ, critic, critic_targ, encoder, encoder_targ],
            get_device_name(),
        )
    ...
    def _update_critic(self, data):
        ...
        def critic_loss():
            ...
            with torch.no_grad():
                ...
                act_targ_noisy = torch.clamp(
                    act_targ_noisy, -self.act_lim, self.act_lim
                )
                # Target Q-values
                q_pi_targ, _ = self.critic_forward(
                    self.critic_targ, obs_next, act_targ_noisy
                )
