...
class MLPGaussianActor(nn.Module):
    def __init__(
        self, obs_dim, act_dim, action_low, action_high, hidden_sizes, activation
    ):
        super().__init__()
        self.action_low = torch.nn.Parameter(
            to_tensor(action_low)[None, ...], requires_grad=False
        )  # (1, act_dim)
        self.action_high = torch.nn.Parameter(
            to_tensor(action_high)[None, ...], requires_grad=False
        )  # (1, act_dim)
...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(
        self, obs, deterministic=False, with_logprob=True, with_distribution=False
    ):
...
class CholeskyGaussianActor(nn.Module):
    ...
    def __init__(
        ...
        self.cholesky_layer = nn.Linear(
            hidden_sizes[-1], (self.act_dim * (self.act_dim + 1)) // 2
        )
        if initializer is not None:
            # init layer weight
    ...
    def forward(self, state):
        ...
        clamped_mean = torch.clamp(
            self.mean_layer(net_out), self.MEAN_CLAMP_MIN, self.MEAN_CLAMP_MAX
        )
        ...
        cholesky_diag_index = (
            torch.div(
                cholesky_diag_index * (cholesky_diag_index + 1),
                2,
                rounding_mode="floor",
            )
            - 1
