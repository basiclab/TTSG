...
from safebench.agent.object_detection.utils.dataloader_label import (
    LoadImagesAndBoxLabels,
)
...
class YoloAgent(BasePolicy):
    def __init__(self, config, logger, train_mode="none") -> None:
        ...
        self.model.warmup(
            imgsz=(1 if self.model.pt else 1, 3, *DEFAULT_CONFIG["imgsz"])
        )
        ...
        if self.mode == "train":
            ...
            hyp[
                "obj"
            ] *= 0  # (self.imgsz[0] / 640) ** 2 * 3 / nl  # scale to image size and layers
            hyp["label_smoothing"] = 0.0
            self.model.model.class_weights = labels_to_class_weights(
                [[[i]] for i in range(15)], len(names_coco128)
            ).to("cuda:0") * len(
                names_coco128
    ...
    def get_action(self, obs, infos, deterministic=False):
        ...
        for i in range(n_envs):
            ...
            pred = non_max_suppression(
                pred, self.conf_thres, self.iou_thres, None, False, max_det=100
            )
        ...
        return [
            {
                "ego_action": np.array([0.2, 0.0]),
                "od_result": pred_list[i],
                "annotated_image": img_annot_list[i],
            }
            for i in range(n_envs)
        ]
    ...
    def _transform_predictions(self, pred):
        if pred[0].shape[0] == 0:
            ...
            pred = {
                "scores": torch.cat([p[:, -2].detach().cpu() for p in pred], dim=-1),
                "labels": [
                    names_coco128[int(idx)]
                    for p in pred
                    for idx in p[:, -1].detach().cpu().numpy()
                ],
...
if __name__ == "__main__":
    agent = YoloAgent(
        {"ego_action_dim": 2, "model_path": None}, None, train_mode="attack"
    )
