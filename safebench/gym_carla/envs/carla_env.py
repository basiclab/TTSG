...
from safebench.scenario.scenario_definition.perception_scenario import (
    PerceptionScenario,
)
...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, env_params, birdeye_render=None, display=None, world=None, logger=None):
        ...
        self.low_speed = env_params["low_speed"]
        ...
        self.discrete_act = [
            env_params["discrete_acc"],
            env_params["discrete_steer"],
        ]  # acc, steer
        ...
        if self.world:
            ...
            if snapshot:
                ...
                if self.scenario_category in ["perception"]:
                    assert isinstance(
                        ego_action, dict
                    ), "ego action in ObjectDetectionScenario should be a dict"
                    world_2_camera = np.array(
                        self.camera_sensor.get_transform().get_inverse_matrix()
                    )
    ...
    def _get_obs(self):
        ...
        if self.scenario_category != "perception":
            ...
            center = (
                int(birdeye_surface.shape[0] / 2),
                int(birdeye_surface.shape[1] / 2),
            )
            ...
            if not self.disable_lidar:
                ...
                self.display.blit(
                    lidar_surface, (self.display_size, self.env_id * self.display_size)
                )
                ...
                self.display.blit(
                    camera_surface,
                    (self.display_size * 2, self.env_id * self.display_size),
                )
            else:
                # display birdeye image
                birdeye_surface = rgb_to_display_surface(birdeye, self.display_size)
                ...
                camera_surface = rgb_to_display_surface(camera, self.display_size)
                self.display.blit(
                    camera_surface, (self.display_size, self.env_id * self.display_size)
                )
    ...
    def _get_reward(self):
        ...
        r_slow = -1 if lspeed_lon < self.low_speed else 0

        # cost for lateral acceleration
        r_lat = -abs(self.ego_vehicle.get_control().steer) * lspeed_lon**2

        # cost for incomplete scenario
        r_route_completion = self.scenario_manager.running_record[-1]["route_complete"]
        r_route_completion = 1 if (r_route_completion > 90) else -1
