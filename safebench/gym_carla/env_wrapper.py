            ...
            if not self.finished_env[e_i]:
                ...
                if done:
                    ...
                    if current_env.config.data_id in self.running_results.keys():
                        self.logger.log(
                            "Scenario with data_id {} is duplicated".format(
                                current_env.config.data_id
                            )
                        )
                    self.running_results[current_env.config.data_id] = (
...
class ObservationWrapper(gym.Wrapper):
    ...
    def _preprocess_obs(self, obs):
        ...
        # concat the 4-dimensional state space and lane info
            ...
            new_obs = np.array(
                [
                    ...
                    obs["state"][3],
                    obs["command"],
                    obs["forward_vector"][0],
                    obs["forward_vector"][1],
                    obs["node_forward"][0],
                    obs["node_forward"][1],
                    obs["target_forward"][0],
                    obs["target_forward"][1],
                ]
            )
            return new_obs
        # return a dictionary with bird-eye view image and state
            ...
            return {
                "img": obs["birdeye"],
                "states": obs["state"][:4].astype(np.float64),
            }
