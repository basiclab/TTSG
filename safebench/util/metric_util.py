...
def cal_out_of_road_length(sequence):
    ...
    for i, out in enumerate(out_of_road):
        ...
        if out:
            total_length += (
                sequence[i]["driven_distance"] - sequence[i - 1]["driven_distance"]
            )
...
def get_route_scores(record_dict):
    ...
    for data_id, sequence in record_dict.items():
        ...
        for time_stamp in sequence:
            avg_acc += math.sqrt(
                time_stamp["ego_acceleration_x"] ** 2
                + time_stamp["ego_acceleration_y"] ** 2
                + time_stamp["ego_acceleration_z"] ** 2
            )
    ...
    ns = {
        metric: (
            score
            if metric not in predefined_max_values
            else score / predefined_max_values[metric]
        )
        for metric, score in scores.items()
    }

    final_score = (
        (1 - ns["collision_rate"]) * 5
        + (
            3
            - ns["avg_red_light_freq"]
            - ns["avg_stop_sign_freq"]
            - ns["out_of_road_length"]
        )
        * 1
        + (
            ns["route_following_stability"]
            + ns["route_completion"]
            + 1
            - ns["avg_time_spent"]
        )
...
def _get_pr_curve(conf_scores, logits, num_gt, data_id, iou_thres=0.5):
    ...
    tp_fp = torch.cumsum(logits >= -0.0, dim=0)
...
def get_perception_scores(record_dict):
    ...
    for data_id in record_dict.keys():
        ...
        for th in np.arange(0.5, 1.0, 0.05):
            map_iou.append(
                _get_pr_curve(conf_scores, logits, num_gt, data_id, iou_thres=th)
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--record_file", default="/home/carla/output/testing_records/record.pkl"
    )
