...
class Logger:
    ...
    def __init__(
        ...
        if osp.exists(self.output_dir):
            self.log(
                ">> Log path %s already exists! Storing info there anyway."
                % self.output_dir,
                "yellow",
            )
        else:
    ...
    def create_eval_dir(self, load_existing_results=True):
        ...
        if load_existing_results:
            if os.path.exists(self.record_file):
                self.log(
                    f">> Loading existing evaluation records from {self.record_file}, "
                )
                self.eval_records = joblib.load(self.record_file)
            else:
                self.log(
                    f">> Loading existing record fail because no records.pkl is found."
                )
                self.eval_records = {}
    ...
    def save_eval_results(self, name=None):
        if name == None:
            ...
            joblib.dump(self.eval_results, self.result_file)
            self.log(
                f">> Saving evaluation records to {self.record_file}, length: {len(self.eval_records)}"
            )
            joblib.dump(self.eval_records, self.record_file)
        else:
            result_dir = os.path.join(self.output_dir, "eval_results")
