...
def get_torch_device():
    ...
    try:
        ...
        raise ValueError(
            "'MODEL_DEVICE' env variable has not been specified. Current 'MODEL_DEVICE' env variable is {device_name}"
        )
        ...
        for k, v in item.items():
            if k in ignore_keys:
                ...
                new_data[k] = to_tensor(
                    v, dtype, device, ignore_keys, transform_scalar, squeeze=squeeze
                )
        return new_data
