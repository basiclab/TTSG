...
from safebench.scenario.scenario_data_loader import CarlaClientDataLoader
from safebench.scenario.scenario_manager.carla_data_provider import CarlaDataProvider
from safebench.scenario.tools.scenario_utils import json_parse
...
class TextToSceneRunner:
    ...
    def _init_world(self, town_name):
            ...
            while not self.env.all_scenario_done():
                ...
                if self.save_video:
                    if self.scenario_category in ["planning", "scenic"]:
                        ...
                        self.logger.add_frame(
                            {
                                s_i["scenario_id"]: ego_actions[n_i]["annotated_image"]
                                for n_i, s_i in enumerate(infos)
                            }
                        )
    ...
    def run(self, test_epoch=None):
        ...
        config_list = json_parse(self.scenario_config, self.logger)
        ...
        for config in config_list:
            ...
            if last_town != config.town:
                self._init_world(config.town)
                self._init_renderer()
                last_town = config.town
            ...
            data_loader = CarlaClientDataLoader(self.world, config, self.num_scenario)
