...
class PerceptionScenario(RouteScenario):
    ...
    def __init__(self, world, config, ego_id, ROOT_DIR, logger):
        ...
        self.object_dict = dict(
            stopsign=list(
                filter(lambda k: "BP_Stop" in k, world.get_names_of_all_objects())
            ),
            car=list(
                filter(
                    lambda k: "SM_Tesla" in k or "SM_Jeep" in k,
    ...
    def get_running_status(self, running_record):
        ...
        if self.config.scenario_id != 0:  # only check when evaluating
            if running_status["route_complete"] == 100:
                stop = True
                self.logger.log(
                    ">> Scenario stops due to route completion", color="yellow"
                )
        else:
            if (
                len(running_record) >= self.max_running_step
            ):  # stop at max step when training
                stop = True
        ...
        for scenario in self.list_scenarios:
            if self.config.scenario_id != 0:  # only check when evaluating
                if (
                    running_status["driven_distance"]
                    >= scenario.ego_max_driven_distance
                ):
                    stop = True
                    self.logger.log(
                        ">> Scenario stops due to max driven distance", color="yellow"
                    )
                    break
    ...
    def get_bbox(self, world_2_camera, image_w, image_h, fov):
        ...
        self.bbox_ground_truth["stopsign"] = self.world.get_level_bbs(
            carla.CityObjectLabel.TrafficSigns
        )
        self.bbox_ground_truth["car"] = self.world.get_level_bbs(
            carla.CityObjectLabel.Vehicles
        )
        self.bbox_ground_truth["person"] = self.world.get_level_bbs(
        ...
        for key, bounding_box_set in self.bbox_ground_truth.items():
            ...
            for bbox in bounding_box_set:
                if (
                    bbox.location.distance(self.ego_vehicle.get_transform().location)
                    < 50
                ):
                    ...
                    if forward_vec.dot(ray) > 1.0:
                        ...
                        self.ground_truth_bbox[key].append(
