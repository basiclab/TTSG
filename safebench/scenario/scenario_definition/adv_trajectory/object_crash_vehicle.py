...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        ...
        super(DynamicObjectCrossing, self).__init__(
            "DynamicObjectCrossing-AdvTraj", config, world
        )
    ...
    def _spawn_blocker(self, transform, orientation_yaw):
        ...
        spawn_point_wp = (
            self.ego_vehicle.get_world().get_map().get_waypoint(transform.location)
        )
    ...
    def initialize_actors(self):
        ...
        forward_vector = (
            self.actor_transform_list[0].rotation.get_forward_vector()
        ...
        self.other_actor_final_transform = carla.Transform(
            self.actor_transform_list[0].location, self.actor_transform_list[0].rotation
        )
        self.other_actor_final_transform.location += forward_vector
        for i in range(self.total_steps):
            self.planned_actor_transform_list.append(
                carla.Transform(
                    carla.Location(
                        self.actor_transform_list[0].location
                        + forward_vector * i / self.total_steps
                    ),
                    self.actor_transform_list[0].rotation,
                )
            )
        for i in range(self.total_steps):
            self.perturbed_actor_transform_list.append(
                carla.Transform(
                    carla.Location(
                        self.planned_actor_transform_list[i].location
                        + right_vector * self.control_seq[i]
                    ),
                    self.actor_transform_list[0].rotation,
                )
            )
    ...
    def update_behavior(self, scenario_action):
        assert (
            scenario_action is None
        ), f"{self.name} should receive [None] action. A wrong scenario policy is used."
        target_transform = self.perturbed_actor_transform_list[
            self.step if self.step < self.total_steps else -1
        ]
        self.step += 1  # <= 60 steps
        self.scenario_operation.drive_to_target_followlane(
            0, target_transform, self._other_actor_target_velocity
        )
    ...
    def check_stop_condition(self):
        ...
        cur_distance = calculate_distance_transforms(
            CarlaDataProvider.get_transform(self.other_actors[0]), self.transform
        )
