...
class OtherLeadingVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=80):
        ...
        super(OtherLeadingVehicle, self).__init__(
            "OtherLeadingVehicle-AdvTraj", config, world
        )
    ...
    def initialize_actors(self):
        ...
        self.second_vehicle_transform = carla.Transform(
            second_vehicle_waypoint.transform.location,
            second_vehicle_waypoint.transform.rotation,
        )

        forward_vector = (
            self._first_actor_transform.rotation.get_forward_vector()
        ...
        self.other_actor_final_transform = carla.Transform(
            self._first_actor_transform.location, self._first_actor_transform.rotation
        )
        ...
        for i in range(len(self.other_actors)):
            if i == 0 and self.need_decelerate:
                ...
                self.scenario_operation.drive_to_target_followlane(
                    i, target_transform, self.dece_target_speed
                )
            else:
                self.scenario_operation.go_straight(
                    self.other_actor_speed[i], i, throttle_value=2.0
                )
