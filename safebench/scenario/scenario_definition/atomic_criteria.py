        ...
        collision_event.set_dict(
            {
                ...
                "x": actor_location.x if actor_location is not None else 0,
                "y": actor_location.y if actor_location is not None else 0,
                "z": actor_location.z if actor_location is not None else 0,
            }
        )
...
class InRouteTest(Criterion):
    ...
    def update(self):
        ...
        for index in range(self._current_index, self._route_length):
            ...
            distance = math.sqrt(
                ((location.x - ref_waypoint.x) ** 2) + ((location.y - ref_waypoint.y) ** 2)
            )
