...
class Detection_Pedestrian(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        ...
        self.object_list = list(
            filter(lambda k: "AD" in k, world.get_names_of_all_objects())
        )
        self.image_path_list = [config.texture_dir]
        self.image_list = [
            cv2.imread(image_file) for image_file in self.image_path_list
        ]
        self.image_list = [
            cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in self.image_list
        ]
        resized = cv2.resize(
            self.image_list[0], (1024, 1024), interpolation=cv2.INTER_AREA
        )
        resized = np.rot90(resized, k=1)
        self.resized = cv2.flip(resized, 1)

        super(Detection_Pedestrian, self).__init__(
            "Detection_Pedestrian", config, world
        )
    ...
    def create_behavior(self, scenario_init_action):
        if self.ego_id == 0:
            inputs = np.array(
                scenario_init_action["image"].detach().cpu().numpy() * 255, dtype=np.int
            )[0].transpose(1, 2, 0)
            height = 1024
            ...
            for o_name in self.object_list:
                ...
                self.world.apply_color_texture_to_object(
                    o_name, carla.MaterialParameter.Diffuse, texture
                )
        else:
            return
    ...
    def _try_spawn_random_walker_at(self):
        ...
        transform_list = [
            carla.Transform(
                carla.Location(x=-50.00, y=203.00, z=2.0),
                carla.Rotation(pitch=0.00, yaw=-180.0, roll=0.0),
            ),
            carla.Transform(
                carla.Location(x=-50.00, y=5 - 1.750, z=2.0),
                carla.Rotation(pitch=0.00, yaw=-180.0, roll=0.0),
            ),
            carla.Transform(
                carla.Location(x=5 - 5.00, y=50.00, z=2.0),
                carla.Rotation(pitch=0.00, yaw=90.0, roll=0.0),
            ),
            carla.Transform(
                carla.Location(x=5 + 195.00, y=50.00, z=2.0),
                carla.Rotation(pitch=0.00, yaw=90.0, roll=0.0),
            ),
        ]  # CarlaDataProvider._rng.choice(CarlaDataProvider._spawn_points)
        destination_list = [
            carla.Location(x=-60.0, y=203.384308, z=2.0),
