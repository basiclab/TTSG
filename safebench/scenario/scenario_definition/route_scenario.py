...
class RouteScenario:
    ...
    def __init__(self, world, config, ego_id, logger, max_running_step):
        ...
        if self.config.method == "text_to_scene":
            self.list_scenarios = []
        else:
    ...
    def _update_route_and_ego(self, timeout=None):
        ...
        possible_scenarios, _ = RouteParser.scan_route_for_scenarios(
            self.config.town, route, world_annotations, scenario_id=scenario_id
        )
        ...
        assert (
            len(scenarios_definitions) >= 1
        ), f"There should be at least 1 scenario definition in the route"
        ...
        for _, definition in enumerate(scenario_definitions):
            ...
            try:
                scenario_instance = scenario_class(
                    self.world, self.ego_vehicle, route_config, timeout=self.timeout
                )
    ...
    def _create_criteria(self):
        ...
        criteria["driven_distance"] = DrivenDistanceTest(
            actor=self.ego_vehicle,
            distance_success=1e4,
            distance_acceptable=1e4,
            optional=True,
        )
        criteria["average_velocity"] = AverageVelocityTest(
            actor=self.ego_vehicle,
            avg_velocity_success=1e4,
            avg_velocity_acceptable=1e4,
            optional=True,
        )
        ...
        if self.config.scenario_id != 0:  # only check when evaluating
            criteria["distance_to_route"] = InRouteTest(
                self.ego_vehicle, route=route, offroad_max=30
            )
