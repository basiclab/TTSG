...
class OtherLeadingVehicle(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(OtherLeadingVehicle, self).__init__(
            "OtherLeadingVehicle-Behavior-Multiple", config, world
        )
    ...
    def initialize_route_planner(self):
        ...
        forward_vector = (
            self.other_actor_transform[0].rotation.get_forward_vector()
            * self._actor_distance
        )
        ...
        self.target_waypoint = [
            ...
            self.target_transform.rotation.yaw,
        ]
        ...
        self.routeplanner = RoutePlanner(
            self.other_actors[0], self.max_waypt, init_waypoints
        )
    ...
    def initialize_actors(self):
        ...
        self.scenario_operation.initialize_vehicle_actors(
            self.other_actor_transform, self.actor_type_list
        )
    ...
    def update_behavior(self):
        ...
        for i in range(len(self.other_actors)):
            if i == 0 and self.need_decelerate:
                self.waypoints, _, _, _, _, self.vehicle_front = (
                ...
                with torch.no_grad():
                    ...
                    new_state = (
                        ...
                        .unsqueeze(0)
                        .to(self.adv_agent.actor.device)
                    )
                    action = (
                        self.adv_agent.actor.forward(new_state)
                        .detach()
                        .cpu()
                        .numpy()[0]
                    )
                    ## train ##
                    current_velocity = (
                        (action[0] + 1) / 2 * self._other_actor_max_velocity
                    )
                self.scenario_operation.go_straight(
                    current_velocity, 0, steering=action[1] * self.STEERING_MAX
                )
                self.step += 1
            else:
    ...
    def _get_obs(self):
        ...
        projection_y = np.linalg.norm(
            rel_ego_location - projection_x * sv_forward_vector
        ) * np.sign(np.cross(sv_forward_vector, rel_ego_location))

        ego_forward_vector = (
            self.ego_vehicle.get_transform().rotation.get_forward_vector()
        )
        ...
        state = np.concatenate(
            (state[:4], np.array([rel_ego_yaw, projection_x, projection_y]))
        ).astype(float)
        return state
