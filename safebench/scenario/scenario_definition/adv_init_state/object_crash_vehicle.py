...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(DynamicObjectCrossing, self).__init__(
            "DynamicObjectCrossing-Init-State", config, world
        )
        ...
        self._number_of_attempts = (
            20  # Total Number of attempts to relocate a vehicle before spawning
        )
    ...
    def initialize_actors(self):
        ...
        """
        y, yaw, self.trigger_distance_threshold = (
            self.actions
        )  # [0, 1], [-60, 60], [15, 50]
        ...
        while True:
            try:
                ...
                forward_vector = (
                    self.transform.rotation.get_forward_vector()
                    * y
                    * self._reference_waypoint.lane_width
                )
                ...
                self.transform = carla.Transform(
                    ...
                    carla.Rotation(
                        self.transform.rotation.pitch, yaw, self.transform.rotation.roll
                    ),
                )
                ...
                self.reference_actor = self.other_actors[
                    0
                ]  # used for triggering this scenario
