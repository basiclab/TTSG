...
class VehicleTurningRoute(BasicScenario):
    ...
    def __init__(self, world, ego_vehicle, config, timeout=60):
        super(VehicleTurningRoute, self).__init__(
            "VehicleTurningRoute-Init-State", config, world
        )
    ...
    def initialize_actors(self):
        ...
        for i in range(len(entry_wps)):
            x_mean += (
                entry_wps[i].transform.location.x + exit_wps[i].transform.location.x
            )
            y_mean += (
                entry_wps[i].transform.location.y + exit_wps[i].transform.location.y
            )
        ...
        x, y, yaw, self.trigger_distance_threshold = self.convert_actions(
            self.actions, max_x_scale, max_y_scale, x_mean, y_mean
        )
        other_actor_transform = carla.Transform(
            carla.Location(x, y, 0), carla.Rotation(yaw=yaw)
        )
