        ...
        for current_waypoint in current_route:
            for waypoint in route:
                distance = np.linalg.norm(
                    [
                        current_waypoint[0] - waypoint[0],
                        current_waypoint[1] - waypoint[1],
                    ]
                )
...
class ScenicDataLoader:
    ...
    def sampler(self):
        ...
        if len(new_config.trajectory) != 0:
            ...
            new_config.trajectory = self.scenicToCarlaLocation(
                new_config.scene.params["egoTrajectoryPts"]
            )
        else:
