                ...
                for spawned_scenario in self.scenario_list:
                    ...
                    if spawned_scenario.reference_actor:
                        reference_location = CarlaDataProvider.get_location(
                            spawned_scenario.reference_actor
                        )
                    if reference_location:
                        cur_distance = calculate_distance_locations(
                            ego_location, reference_location
                        )

                    if (
                        cur_distance
                        and cur_distance < spawned_scenario.trigger_distance_threshold
                    ):
                        if spawned_scenario not in self.triggered_scenario:
                            self.logger.log(
                                ">> Trigger scenario: " + spawned_scenario.name
                            )
