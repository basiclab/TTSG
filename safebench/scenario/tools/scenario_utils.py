...
from safebench.scenario.tools.route_parser import (
...
def scenario_parse(config, logger):
    ...
    list_of_scenario_config = osp.join(
        ROOT_DIR, config["scenario_type_dir"], config["scenario_type"]
    )
    route_file_formatter = osp.join(
        ROOT_DIR,
        config["route_dir"],
        "scenario_%02d_routes/scenario_%02d_route_%02d.xml",
    )
    scenario_file_formatter = osp.join(
        ROOT_DIR, config["route_dir"], "scenarios/scenario_%02d.json"
    )
...
def scenic_parse(config, logger):
    ...
    for j in search_scenarios:
        ...
        new_files = sorted(
            [
                ...
                if ("." in path) and path.split(".")[1] == "scenic"
            ]
        )
    ...
    try:
        params_dir = open(
            osp.join(
                scenic_dir,
                f"scenario_{config['scenario_id']}.json",
            )
        )
...
def json_parse(config, logger):
    ...
    assert len(scenic_rel_listdir) > 0, "no json file in this dir"
    ...
    for i, scenic_file in enumerate(scenic_abs_listdir):
        ...
        for route_id in config["route_id"]:
            ...
            scenario_file = scenario_file_formatter % config["scenario_id"]
            parsed_configs = RouteParser.parse_routes_file(route_file, scenario_file)
            ...
            parsed_config.auto_ego = config["auto_ego"]
            parsed_config.num_scenario = config["num_scenario"]
            parsed_config.scenario_folder = "adv_behavior_single"
            parsed_config.method = config["method"]
            parsed_config.data_id = i
    ...
    config_by_map = {}
    for config in config_list:
        if config.town not in config_by_map:
            config_by_map[config.town] = []
        config_by_map[config.town].append(config)
    new_lists = list(config_by_map.values())
    config_list = [item for sublist in new_lists for item in sublist]
    return config_list
