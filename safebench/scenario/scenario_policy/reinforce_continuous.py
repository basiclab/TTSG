...
class AutoregressiveModel(nn.Module):
    ...
    def forward(self, x, determinstic):
        ...
        # p(b|a,s)
        state_sample_a = (
            torch.cat((s, mu_a), dim=1)
            if determinstic
            else torch.cat((s, action_a), dim=1)
        )
        ...
        # p(c|a,b,s)
        state_sample_a_b = (
            torch.cat((s, mu_a, mu_b), dim=1)
            if determinstic
            else torch.cat((s, action_a, action_b), dim=1)
        )
        ...
        if self.standard_action_dim:
            state_sample_a_b_c = (
                torch.cat((s, mu_a, mu_b, mu_c), dim=1)
                if determinstic
                else torch.cat((s, action_a, action_b, action_c), dim=1)
            )
        ...
        if self.standard_action_dim:
            action = torch.cat(
                (action_a, action_b, action_c, action_d), dim=1
            )  # [B, 4]
            mu = torch.cat((mu_a, mu_b, mu_c, mu_d), dim=1)  # [B, 4]
            sigma = torch.cat((sigma_a, sigma_b, sigma_c, sigma_d), dim=1)  # [B, 4]
        else:
...
class REINFORCE(BasePolicy):
    ...
    def proceess_init_state(self, state):
        ...
        for s_i in range(len(state)):
            ...
            processed_state = np.concatenate(
                (route_norm, [target_speed]), axis=0
            ).astype("float32")
    ...
    def load_model(self, scenario_configs=None):
        ...
        for config in scenario_configs:
            ...
            assert (
                self.standard_action_dim == config.parameters[1]
            ), "Action dimensions should be the same in a batch."

        # TODO: remove this after obtaining new models with consistent action dim
        self.model = CUDA(
            AutoregressiveModel(self.num_waypoint, self.standard_action_dim)
        )
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr)
        ...
        if os.path.exists(model_filename):
            ...
            self.logger.log(
                f">> Fail to find lc model from {model_filename}", color="yellow"
            )
