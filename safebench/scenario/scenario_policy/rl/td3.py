...
class TD3(BasePolicy):
    ...
    def __init__(self, config, logger):
        ...
        # aka critic
        self.q_funcs = CUDA(
            DoubleQFunc(self.state_dim, self.action_dim, hidden_size=self.hidden_size)
        )
        ...
        # aka actor
        self.policy = CUDA(
            Policy(self.state_dim, self.action_dim, hidden_size=self.hidden_size)
        )
    ...
    def update_target(self):
        ...
        with torch.no_grad():
            ...
            for target_pi_param, pi_param in zip(
                ...
                target_pi_param.data.copy_(
                    self.tau * pi_param.data + (1.0 - self.tau) * target_pi_param.data
                )

    def update_q_functions(
        self, state_batch, action_batch, reward_batch, nextstate_batch, done_batch
    ):
        with torch.no_grad():
    ...
    def train(self, replay_buffer):
        ...
        for _ in range(self.update_iteration):
            ...
            bn_s = CUDA(torch.FloatTensor(batch["actor_info"])).reshape(
                self.batch_size, -1
            )
            bn_s_ = CUDA(torch.FloatTensor(batch["n_actor_info"])).reshape(
                self.batch_size, -1
            )
    ...
    def save_model(self, episode):
        ...
        filepath = os.path.join(
            self.model_path, f"model.td3.{self.model_id}.{episode:04}.torch"
        )
    ...
    def load_model(self, episode=None):
        ...
        if os.path.isfile(filepath):
            self.logger.log(
                f">> Loading scenario policy {self.name} model from {filepath}"
            )
            ...
            self.logger.log(
                f">> No scenario policy {self.name} model found at {filepath}", "red"
            )
            exit()
