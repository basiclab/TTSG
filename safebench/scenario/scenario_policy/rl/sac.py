...
class SAC(BasePolicy):
    ...
    def train(self, replay_buffer):
        ...
        for _ in range(self.update_iteration):
            ...
            sample_action, log_prob, z, batch_mu, batch_log_sigma = (
                self.get_action_log_prob(bn_s)
            )
            ...
            for target_param, param in zip(
                ...
                target_param.data.copy_(
                    target_param * (1 - self.tau) + param * self.tau
                )
    ...
    def save_model(self, episode):
        ...
        filepath = os.path.join(
            save_dir, f"model.sac.{self.model_id}.{episode:04}.torch"
        )
        ...
        if scenario_configs is not None:
            for config in scenario_configs:
                ...
                model_filename = os.path.join(
                    self.model_path, str(scenario_id), model_file
                )
                if os.path.exists(model_filename):
                    self.logger.log(
                        f">> Loading {self.policy_type} model from {model_filename}"
                    )
                    ...
                    self.logger.log(
                        f">> Fail to find {self.policy_type} model from {model_filename}",
                        color="yellow",
                    )
