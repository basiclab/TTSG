...
class ObjectDetection(object):
    ...
    def get_init_action(self, obs=None, deterministic=False):
        ...
        self._img = img
        return [{"attack": eps, "image": img} for _ in range(len(obs))], [
            {} in range(len(obs))
        ]
...
if __name__ == "__main__":
    ...
    cv2.imwrite(
        "demo.jpg",
        np.array(ret["image"].detach().cpu().numpy() * 255, dtype=np.int)[0].transpose(
            1, 2, 0
        ),
    )
