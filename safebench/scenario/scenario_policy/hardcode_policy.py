        ...
        for config in scenario_configs:
            ...
            if isinstance(parameters, str):
                ...
                if os.path.exists(model_filename):
                    self.logger.log(
                        f">> Loading {self.scenario_type} model from {model_filename}"
                    )
                    with open(model_filename, "r") as f:
                        self.parameters.append(json.load(f))
                else:
                    self.logger.log(
                        f">> Fail to find {self.scenario_type} model from {model_filename}",
                        color="yellow",
                    )
                ...
                self.logger.log(
                    f">> Fail to find {self.scenario_type} parameters", color="yellow"
                )
