...
class NormalizingFlow(REINFORCE):
    ...
    def __init__(self, scenario_config, logger):
        ...
        self.model_path = os.path.join(
            scenario_config["ROOT_DIR"], scenario_config["model_path"]
        )
        ...
        # latent space
        self.z = MultivariateNormal(
            CUDA(torch.zeros(self.action_dim)), CUDA(torch.eye(self.action_dim))
        )

        # prior model and generator
        self.prior_model = CUDA(
            RealNVP(n_flows=self.n_flows, data_dim=self.action_dim, n_hidden=128)
        )
        self.model = CUDA(
            ConditionalRealNVP(
    ...
    def train_prior_model(self, prior_data):
        ...
        train_loader = torch.utils.data.DataLoader(
            prior_data, shuffle=True, batch_size=self.batch_size
        )
        self.prior_optimizer = optim.Adam(
            self.prior_model.parameters(), lr=self.prior_lr
        )
        ...
        for epoch in range(self.prior_epochs):
            ...
            for data in train_loader:
                ...
                loglikelihood = -torch.mean(
                    self.z.log_prob(sample_z)[:, None] + log_det_jacobian
                )
            ...
            self.logger.log(
                "[{}/{}] Prior training error: {}".format(
                    epoch, self.prior_epochs, np.mean(avg_loglikelihood)
                )
            )
    ...
    def prior_sample(self, sample_number=1000, sigma=1.0):
        sampler = MultivariateNormal(
            CUDA(torch.zeros(self.action_dim)), CUDA(sigma * torch.eye(self.action_dim))
        )
    ...
    def train(self, replay_buffer):
        ...
        for _ in range(self.itr_per_train):
            ...
            loss_r = -(
                loglikelihood * (torch.exp(-episode_reward) + self.alpha * prior_prob)
            ).mean()
    ...
    def load_model(self, scenario_configs=None):
        ...
        ), "Scenario configs should be provided for loading model."
        scenario_id = scenario_configs[0].scenario_id
        model_file = scenario_configs[0].parameters[0]
        for config in scenario_configs:
            assert (
                scenario_id == config.scenario_id
            ), "Scenarios should be the same in a batch."
            assert (
                model_file == config.parameters[0]
            ), "Model filenames should be the same in a batch."
        ...
        if os.path.exists(model_filename):
            ...
            self.logger.log(
                f">> Fail to find nf model from {model_filename}", color="yellow"
            )

    def save_prior_model(self):
        states = {"parameters": self.prior_model.state_dict()}
        model_filename = os.path.join(
            self.model_path, "nf.prior." + str(self.model_id) + ".pt"
        )
        with open(model_filename, "wb+") as f:
            torch.save(states, f)
    ...
    def load_prior_model(self):
        ...
        if os.path.isfile(model_filename):
            ...
            self.logger.log(
                f">> Fail to find nf prior model from {model_filename}", color="yellow"
            )
