...
class AdvMultiAgentReplayBuffer:
    def __init__(
        ...
        self.collision_buffer = MultiAgentReplayBuffer(
            max_size, critic_dims, actor_dims, n_actions, n_agents, batch_size // 2
        )
        self.non_collision_buffer = MultiAgentReplayBuffer(
            max_size, critic_dims, actor_dims, n_actions, n_agents, batch_size // 2
        )
    ...
    def store_transition(
        ...
        if collision:
            self.collision_buffer.store_transition(
                raw_obs, state, action, next_action, reward, raw_obs_, state_, done
            )
        else:
            self.non_collision_buffer.store_transition(
                raw_obs, state, action, next_action, reward, raw_obs_, state_, done
            )
    ...
    def sample_buffer(self):
        (
            ...
            states_,
            terminal,
        ) = self.collision_buffer.sample_buffer()
        (
            actor_states2,
            states2,
            actions2,
            ...
            states_2,
            terminal2,
        ) = self.non_collision_buffer.sample_buffer()
        ...
        for agent_idx in range(self.n_agents):
            actor_states[agent_idx] = np.concatenate(
                (actor_states[agent_idx], actor_states2[agent_idx]), axis=0
            )
            actor_new_states[agent_idx] = np.concatenate(
                (actor_new_states[agent_idx], actor_new_states2[agent_idx]), axis=0
            )
            actions[agent_idx] = np.concatenate(
                (actions[agent_idx], actions2[agent_idx]), axis=0
            )
            next_actions[agent_idx] = np.concatenate(
                (next_actions[agent_idx], next_actions2[agent_idx]), axis=0
            )
        return (
            actor_states,
            states,
...
class MultiAgentReplayBuffer:
    ...
    def store_transition(
        self, raw_obs, state, action, next_action, reward, raw_obs_, state_, done
    ):
