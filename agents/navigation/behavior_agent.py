...
class BehaviorAgent(BasicAgent):
    ...
    def __init__(
        ...
        # Vehicle information
        self._interference_distance = interference_distance
        self._speed = 0
        self._speed_limit = 0
    ...
    def run_step(self, debug=False, ignore_speed_limit=False):
        """
        ...
        move_fast = False
        if self.interfere_target is not None and self.interfere_target._vehicle.is_alive:
            distance_ego_to_target = self.interfere_target._vehicle.get_location().distance(
                self.interfere_target.destination_waypoint.transform.location
            )
            distance_agent_to_target = ego_vehicle_loc.distance(
                self.interfere_target.destination_waypoint.transform.location
            )

            if distance_ego_to_target - distance_agent_to_target > DISTANCE_FOR_INTERFERENCE:
                return carla.VehicleControl(brake=1.0, throttle=0)
            elif distance_agent_to_target > distance_ego_to_target or (
                distance_ego_to_target - distance_agent_to_target < 3
            ):
                move_fast = True
            elif (
                distance_agent_to_target < distance_ego_to_target
                and distance_ego_to_target - distance_agent_to_target < DISTANCE_FOR_INTERFERENCE
            ):
        ...
        # 4: Normal behavior
            ...
            speed_limit_list = [self._behavior.max_speed]
            if not ignore_speed_limit:
                speed_limit_list.append(self._speed_limit - self._behavior.speed_lim_dist)
            if not move_fast:
                target_speed = min(speed_limit_list)
            else:
                target_speed = 120
            self._local_planner.set_speed(target_speed)
            control = self._local_planner.run_step(debug=debug)
